...
const addCheckBox = (element: HTMLElement) => {
  ...
  checkbox.addEventListener("click", function (e) {
    ...
    this.parentElement.style.backgroundColor = checkbox.checked
      ? CONFIG.CONTENT_BG
      : ""
    this.parentElement.style.border = checkbox.checked
      ? `1px solid ${CONFIG.CONTENT_BORDER}`
      : "none"
  })
...
const observeNode = (element) => {
  ...
  element.addEventListener("click", function (e) {
    ...
    if (checkbox) {
      ...
      this.style.border = checkbox.checked
        ? `1px solid ${CONFIG.CONTENT_BORDER}`
        : "none"
    }
  })
...
const SelectionAreaScan = async () => {
  ...
  const selectables = tags
    .map((item) => `body #main article .markdown > ${item}`)
    .join(",")
  const liSelectables = liTags
    .map((item) => `body #main article .markdown > ${item} li`)
    .join(",")
...
function userSelectNone() {
  document.body.style.userSelect = "none"
}

function userSelectRestore() {
  document.body.style.userSelect = ""
}

let ac = null

const enableUserSelectLock = () => {
  if (ac) return // 已启用
  ac = new AbortController()
  const signal = ac.signal
  document.addEventListener("mousedown", userSelectNone, { signal })
  document.addEventListener("mouseup", userSelectRestore, { signal })
}

const disableUserSelectLock = () => {
  if (!ac) return
  ac.abort() // 自动移除所有通过该 signal 注册的监听器
  ac = null
}
...
export default function UI() {
  ...
  const statusControl = (status: boolean) => {
    if (status) {
      selection?.enable()
    } else {
      // selection?.clearSelection()
  ...
  const userSelectControl = (status) => {
    if (status) {
      enableUserSelectLock()
    } else {
      disableUserSelectLock()
    }
  }
  ...
  useEffect(() => {
    statusControl(isActive)
    userSelectControl(isActive)
  }, [isActive])

  return (
    <div className="fixed bottom-1/12 right-1/12 z-[9999] flex flex-col">
      <div className="mb-2">
        <button
          onClick={clearControl}
          className={`px-4 py-1 text-white rounded ${isActive ? "hover:opacity-80 bg-gray-400 cursor-pointer " :"bg-gray-300"}`}>
          清除
        </button>
      </div>
      <div className="mb-2">
        <button
          onClick={copyControl}
          className={`px-4 py-1 text-white rounded ${isActive ? "bg-theme hover:opacity-80 cursor-pointer " : "bg-gray-300"}`}>
          拷贝
        </button>
      </div>
      <div className="flex items-center mb-2">
        <button
          ...
          className={`cursor-pointer hover:opacity-80 px-4 py-1 text-white rounded ${isActive ? "bg-gray-300" : "bg-green-600"} `}>
          {isActive ? "停用" : "激活"}
        </button>
        <div className="ml-4">
        {
          isActive ? <span className="relative flex size-2">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-green-400 opacity-75"></span>
            <span className="relative inline-flex size-2 rounded-full bg-green-500"></span>
          </span> : <span className="flex size-2 rounded-full bg-gray-300"></span>
        }
        </div>
      </div>
    </div>
